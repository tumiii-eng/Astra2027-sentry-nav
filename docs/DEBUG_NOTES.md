...
拟定修法：给 `global_obstacle_map_node` 加 `dynamic_intensity_max` 参数（对齐 B 套的 2.0），`inten >= dyn_intensity_max_` 也跳过。改完重编 `astra_pb2025_bridge`，多轮往返测 (0,0) ↔ (17,2) 确认不飞车、路径变忠诚。

早期把这个当成「偶发的定位漂移」处理是误判，它是可复现的正反馈崩溃，有明确的 A 套侧根因。

### 目标处理链只有单点判据，没有连通性判据

`goal_reachability_resolver` / `local_goal_selector` / `endpoint_projector` 都只判「目标这个点离障碍够不够远」，目标单点合法就原样送给 JPS，从不判起点到目标之间是不是连通的。合法但不连通的远目标必然规划失败。

治本方向：目标处理链里加一步「从起点在膨胀图上做 BFS 连通性检查」，不连通时回退到朝目标方向最远的那个可达自由格。
...
实测 (12,2) 到达 (11.977, 1.904)，JPS 失败 0 次；基线 (5,-5) 也正常。这是治标，治本还是上面那条连通性判据，或者改成软代价膨胀。场景更复杂的时候再说。
...
改成用预建的 `/map` 作障碍源（就是现在的 `global_obstacle_map_node`），占据率恢复到 46%（含膨胀）。顺带解决了另外两个问题：Astra 原本不订阅 `/map`，所以不知道先验地图和手动标注的障碍；原本的滚动局部图只有 10×10m，没法全局规划。现在基于全场 `/map` 在 map 帧规划，ESDF / JPS / MINCO 自然覆盖全场。
...
改法：`controller_node` 的 `on_timer` 在无 odom / 无轨迹时**不发布** cmd_vel。静默让 `fake_vel_transform` 判超时、不自旋。已验证发目标前完全静止。这是唯一改过的 Astra 源码。
